...
def main():
    ...
    def make_df(filename, label):
        ...
        df['Atomic Symbol'] = df['Atomic Number'].apply(atomic_number_to_symbol)
        df['Account'] = [label]*len(df)
        return df

    df_atomic_number = pd.read_csv("atomic_number.csv")
    atomic_dic = {n: s for n, s in zip(df_atomic_number['Number'], df_atomic_number['Symbol'])}
    ...
    fig = px.scatter(df, x='Date', y='Atomic Number', text='Atomic Symbol', size_max=60, color='Account')
    fig.update_traces(textposition='top center', textfont_size=24,marker={"size":12})
